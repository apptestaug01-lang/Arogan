    ...
    <div className="space-y-10">
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm text-primary-100">
              Apply for corporate lending in one clear, secure workspace. Pick a loan type below to
              see exactly which documents you'll need, then start your application.
            </p>
          </div>
          <Button
            variant="secondary"
      ...
      <section>
        ...
        <p className="page-sub">
          Required documents vary by facility — review them before you begin.
        </p>
      ...
      <section>
        ...
        <p className="page-sub">
          Keep this list handy — every document you may need across the application lifecycle.
        </p>
          ...
          <TabsList className="grid w-full grid-cols-2 gap-1 sm:grid-cols-4">
            {requiredDocumentsMaster.map((section) => (
              <TabsTrigger key={section.id} value={section.id}>
                {tabLabels[section.id] ?? section.title}
              </TabsTrigger>
            ))}
          </TabsList>
          {requiredDocumentsMaster.map((section) => (
