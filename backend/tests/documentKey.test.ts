...
describe('buildDocumentKey', () => {
  it('builds a user-scoped key from the given ids', () => {
    const key = buildDocumentKey('user-1', 'app-1', 'doc-1', 'aadhar.pdf');
    expect(key).toBe('borrowers/user-1/applications/app-1/documents/doc-1/aadhar.pdf');
  });

  it('strips path-traversal and unsafe characters from every segment', () => {
    const key = buildDocumentKey('user/../x', 'app', 'doc', '../../evil.exe')
